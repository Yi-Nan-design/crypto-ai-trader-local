...
独立的 Shadow 学习链。它优先从 `0.54` 到 `0.70` 的较低原始概率
阈值中寻找信号，再使用趋势、区间、成交量、流动性和波动条件过滤。
过滤后的策略必须在验证校准段、后续验证门控段保持正收益，测试集
只做一次最终发布否决。支持做多和做空。信号触发后按通过验证的
预测周期保持模拟方向；风险关闭、流动性不足、资金费率拥挤等硬门禁
仍可提前退出。

Shadow 合约模拟默认使用 `10x` 逐仓。单币种保证金比例最多 `2.5%`，
对应单币种名义敞口最多 `25%`，组合名义总敞口最多 `50%`。这不是
全仓 `10x`，并继续保留爆仓、回撤、流动性、资金费率和市场状态
保护。Shadow 结果不会反向放宽严格策略，也不会连接实盘执行。
